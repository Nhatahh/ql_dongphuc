    ...
    public function showDetail($id_loai)
{
    $ct_sp = DB::select("
        ...
        WHERE `24_loaisanpham`.id = ?

        GROUP BY `24_loaisanpham`.id
    ", [$id_loai]);

    if (!$ct_sp || count($ct_sp) === 0) {
        abort(404, 'Không tìm thấy sản phẩm');
    }

    $ct_sp = $ct_sp[0]; // Lấy object đầu tiên để view xử lý dễ
    $sanphams = $this->getSanpham();
    return view('dongphuc.uniforms.show_detail', compact('ct_sp','sanphams'));
}
