...
@foreach ($tiepnhan as $value)
@php
    $i++;
@endphp
{{-- <div style="padding-top: -60px; text-align:right; font-family: 'Times New Roman', Times, serif; font-size: 10pt; font-style:italic">
    {{$value->mp}}
</div> --}}
...
<table>
    <tbody>
        ...
        <tr>
            <td colspan="2" class="border">
                <div>Hộ khẩu thường trú: &nbsp; {{$value->huyen}}, {{$value->tinh}}</div>
            </td>
        </tr>
        <tr>
            <td colspan="2" class="border">
    ...
    <tbody>
        {{-- <tr>
            ...
            <td class="center border" width="50%">
                <div><i>Cần Thơ, ngày @if ($value->day < 10) 0{{$value->day}} @else {{$value->day}} @endif tháng @if ($value->month < 2) 0{{$value->month}} @else {{$value->month}} @endif năm {{$value->year}}</i></div>
            </td>
        </tr> --}}
        <tr>
            <td class="center border" width="50%"></td>
