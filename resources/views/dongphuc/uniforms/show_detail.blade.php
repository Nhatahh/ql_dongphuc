        ...
        <div class="product-detail row mt-4">
            ...
            <div class="product-img col-12 col-md-6">
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-lg">
                        <div class="modal-content bg-transparent border-0">
                            <div class="modal-body text-center p-0">
                                <img id="modalImage" src="{{ $ct_sp->anhsanpham }}" 
                ...
                <div class="product-img__extra row mt-3 flex-nowrap overflow-auto">
                    <img src="{{ $ct_sp->anhsanpham }}" alt="" class="product-img__extra-item img-fluid col-3 thumbnail" style="cursor:pointer">
            ...
            <div class="col-12 col-md-6">
                <div class="product-info col-12 ps-md-5 mt-md-0 d-flex flex-column mt-4">
                    <div class="fs-1 mb-3"><strong>{{ $ct_sp->loai }}</strong></div>
                    <div class="d-flex flex-column">
                        <p>Loại: <span class="product-info__cate">{{ $ct_sp->loai }}</span></p>
                        <p>Nhà sản xuất: <span class="product-info__nsx">{{ $ct_sp->nsx }}</span></p>
                        <div class="d-flex w-50 justify-content-between">
                            <!-- <p>Tồn kho: <span class="product-info__stock" >{{ $ct_sp->tonkho }}</span></p> -->
                            <small class="text-muted"
                                id="tonkho_{{ $ct_sp->id_loai }}"
                                data-tonkho="{{ $ct_sp->tonkho }}">
                                {{ $ct_sp->tonkho }} sản phẩm còn lại
                            </small>
                            <small class="text-muted">Đã bán: {{ $ct_sp->slphat  }}</small>
                        </div>
                    </div>
                    ...
                    <div class="gap-3 d-flex justify-content-center mt-3">
                        <div class="input-group quantity-group" style="max-width: 100px;">
                            ...
                            <input type="number"
                                ...
                                value="1" min="1"
                                data-sp-id="{{ $ct_sp->id_loai }}" id="soluong_{{ $ct_sp->id_loai }}">
                            <button class="btn btn-outline-secondary btn-sm" type="button" onclick="changeQuantity(this, 1)">
                                <i class="fas fa-plus"></i>
                            </button>
                        </div>
                        <!-- select 2 size -->
                        <select id="sizeSelect2" class="form-select sizeSelect2" style="max-width: 200px;" data-sp-id="{{ $ct_sp->id_loai }}">
                            <option value="">--- Chọn size ---</option>
                        </select>            
                    </div>
                    <div class="gap-3 d-flex justify-content-center mt-3">
                        <button class="btn btn-danger fs-4 w-50" data-sp-id="{{ $ct_sp->id_loai }}" data-url=""><i class="bi bi-bag-check-fill"></i> Mua ngay</button>
                        <button class="btn btn-primary btn-addSP fs-4 w-50" data-sp-id="{{ $ct_sp->id_loai }}" data-url="{{ route('addSP') }}" data-cart-url="{{ route('orders.cart') }}"><i class="fa-solid fa-cart-plus"></i> Thêm giỏ hàng</button>
                    </div>
                </div>
                <!-- Description -->
                        ...
                        <div class="col-6 col-md-3 mt-4 d-flex">
                            <a href="{{ route('uniforms.show_detail', $sp->id_loai) }}" class="text-decoration-none text-dark w-100">
                                <div class="card h-100 d-flex flex-column shadow-sm border-0 position-relative hover-shadow" style="transition: transform 0.3s ease;">
                                    ...
                                    <img src="{{  $sp->anhsanpham }}" class="card-img-top" alt="{{ $sp->loai }}"
                                        style="height: 220px; object-fit: cover; border-top-left-radius: 0.5rem; border-top-right-radius: 0.5rem;">

                                    <div class="card-body d-flex flex-column">
                                        <h5 class="card-title text-truncate" title="{{ $sp->loai }}">{{ $sp->loai }}</h5>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mt-auto">
                                            ...
                                            <p class="fs-6 mb-0">Đã bán: {{ $sp->slphat ?? 0 }}</p>
                                        </div>
                                    </div>
                                </div>
                            </a>
