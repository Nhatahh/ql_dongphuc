    ...
    $.ajax({
        ...
        success:async function (res) {
            if(['ins_1','rol_2'].includes(res.trangthai) == true){
                ...
                const idMoi = res.id_moi; // Lấy id_moi từ phản hồi
                const logoPath = "/img/CTUT_logo.jpg"; // Đường dẫn logo
                const code_QR = `${ma_loai}.${ma_nhasanxuat}.${ma_size}.${idMoi}`; // Chuỗi nội dung QR code
                const response = await taoQRCodeVaGuiAjax(code_QR, logoPath, id_chucnang);
                if (response.trangthai === "ins_1") {
...
// tạo qr code
async function taoQRCodeVaGuiAjax(data, logoPath, id_chucnang, width = 400, height = 400, margin = 0) {
    return new Promise(async (resolve, reject) => {
        try {
            // Tạo mã QR code
            const qrCode = new QRCodeStyling({
                width: width, // Kích thước QR code
                height: height,
                data: data, // Nội dung mã QR
                image: logoPath, // Đường dẫn logo
                margin: margin, // Khoảng cách biên
                dotsOptions: {
                    color: "#000000", // Màu của các chấm
                    type: "square", // Kiểu chấm
                },
                backgroundOptions: {
                    color: "#ffffff", // Màu nền
                },
                imageOptions: {
                    crossOrigin: "anonymous", // Hỗ trợ tải logo từ các nguồn khác domain
                    margin: 10, // Khoảng cách logo với QR code
                },
            });

            // Lấy dữ liệu Base64 từ mã QR (dưới dạng Blob)
            const qrBlob = await qrCode.getRawData("png");

            // Chuyển đổi Blob thành chuỗi Base64
            const reader = new FileReader();
            reader.onloadend = function () {
                const base64data = reader.result.split(',')[1]; // Đây là chuỗi Base64 của QR code
                console.log(base64data); // Log chuỗi Base64 của QR code

                // Gửi dữ liệu qua AJAX
                $.ajax({
                    type: "POST",
                    url: "themsanpham_qrcode", // Thay bằng route của bạn
                    data: {
                        qrCode: base64data,                    
                        data: data,
                        idMoi: data.split('.')[3], // Trích xuất idMoi từ data
                        id_chucnang: id_chucnang // Trích xuất idMoi từ data
                    },
                    success: function (response) {
                        resolve(response); // Trả về kết quả nếu thành công
                    },
                    error: function (xhr, status, error) {
                        reject(error); // Báo lỗi nếu có
                    },
                });
            };
            
            // Đọc Blob và chuyển nó thành Base64
            reader.readAsDataURL(qrBlob); 
        } catch (error) {
            reject(error); // Trả về lỗi nếu có ngoại lệ
        }
    });
}
...
async function update_Sanpham(event,id){
    ...
    const logoPath = "/img/CTUT_logo.jpg"; // Đường dẫn logo
    const code_QR = `${ma_loai_up}.${ma_nhasanxuat_up}.${ma_size_up}.${id}`; // Chuỗi nội dung QR code
    const response = await taoQRCodeVaGuiAjax(code_QR, logoPath, id_chucnang);
    if (response.trangthai === "ins_1") {
        $.ajax({
            type:"POST",
