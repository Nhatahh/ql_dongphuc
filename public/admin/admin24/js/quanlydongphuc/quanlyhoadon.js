...
var ds_hoadon = $("#ds_hoadon").DataTable({
    ...
    columns: [
        ...
        {
            title: "<div style='text-align: center;'>CCCD</div><div style='border-top:2px solid #dee2e6;width:100%;display: flex;justify-content: center;align-items: center;position:relative;'><input id='search_cccd' onkeyup='search_cccd()' class='form-control' style='width:90%;height:22px;border:none;padding-right:30px;'><i style='color:#dee2e6;position:absolute;right:5px;top:50%;transform:translateY(-50%);pointer-events:none;' class='fa-solid fa-magnifying-glass'></i></div>",
            data: "cccd",
            className: 'remove_click text-left',
        },
        {
...
function search_tg() {
    ds_hoadon.column(4).search(value = $('#search_tg').val()).draw();
}
function search_dotphat() {
    ds_hoadon.column(5).search(value = $('#search_dotphat').val()).draw();
}
function search_cccd() {
    ds_hoadon.column(3).search(value = $('#search_cccd').val()).draw();
}
