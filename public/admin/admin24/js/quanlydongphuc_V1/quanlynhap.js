...
async function capnhat_soluongnhap(event, id){
    ...
    $.ajax({
        ...
        success: function (res) {
            if(['ins_0', 'ins_1', '-100','dot_0','ins_-1','rol_2'].includes(res.trangthai) == true){
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            data.forEach(item => {
                ...
                sizeData[label][item.size] += parseInt(item.tong_sl_nhap, 10); // Chuyển đổi sang số nguyên
            });
